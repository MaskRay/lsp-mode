...
*** I have multiple language servers registered for language FOO. Which one will be used when opening a project?
    ...
    The one with highest priority. ~lsp-clients.el~ predefined servers have priority -1 while external packages usually have priority 0.
    This makes explicitly required external packages win.
** See also
